...
if (!JWT_REFRESH_SECRET || JWT_REFRESH_SECRET.length < 32) throw new Error('JWT_REFRESH_SECRET must be set and contain at least 32 characters.');
...
function setAuthCookies(res, user) {
    ...
    res.cookie('access_token', signAccessToken(user), { ...options, maxAge: 15 * 60 * 1000 });
    res.cookie('refresh_token', signRefreshToken(user), { ...options, maxAge: 7 * 24 * 60 * 60 * 1000 });
}
...
function authenticateAccess(req, res, next) {
    ...
    try {
        const payload = jwt.verify(token, JWT_SECRET, { issuer: 'sentiment-analysis-dashboard', audience: 'dashboard' });
...
app.post('/api/auth/register', authLimiter, async (req, res) => {
    ...
    if (users.some(user => user.email === email)) return res.status(409).json({ error: 'An account with that email already exists.' });
...
app.post('/api/auth/refresh', async (req, res) => {
    ...
    try {
        const payload = jwt.verify(token, JWT_REFRESH_SECRET, { issuer: 'sentiment-analysis-dashboard', audience: 'dashboard' });
...
app.post('/api/auth/logout', (req, res) => { clearAuthCookies(res); res.status(204).end(); });
...
app.get('/api/sentiment', authenticateAccess, async (_req, res) => {
    try {
        ...
        res.json(data);
    } catch { res.status(500).json({ error: 'Unable to load sentiment data.' }); }
});
...
app.listen(PORT, () => console.log(`Sentiment dashboard listening on http://localhost:${PORT}`));
